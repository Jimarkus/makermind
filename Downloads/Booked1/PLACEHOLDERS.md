...
- **Prices** — $80 per 60-minute session, $280 per month for weekly
...
- **Removed invented credentials.** An early draft claimed "10+ years
  ...
  years 2–12, Canberra — and the About stats table is gone.
...
- **Social links are live.** Instagram and Facebook are wired into the
  Contact page, opening in a new tab with `rel="noopener noreferrer"`. The
  YouTube button was removed rather than left pointing nowhere — say the
  word when a channel exists and I will put it back.
- **Fixed a contradiction on the Contact page.** The details panel still
  promised a reply "within 1 business day" while every other page said two.
  It now says two.

- **Year range changed to 2–12.** Everything previously said Years 7–12.
  The site, the business card, the newsletter ads, the social artwork and
  the Google Business Profile set now all say Years 2&ndash;12. Two things
  were added rather than renamed: a **Beginner robot kits** chip in the
  subject list, and a fifth programme card, **First robots** — beginner
  kits (LEGO, VEX GO, mBot), building and first sensors, and RoboCup Junior
  primary divisions. Pricing is unchanged at every year level, as you asked.

- **New page: `booked.html`.** A real confirmation page that can only load
  after a booking completes. `assets/scheduler.js` listens for Calendly's
  documented `calendly.event_scheduled` message and sends the visitor there,
  because the Calendly iframe never changes the page URL — without it there
  is no way for Google Ads or Analytics to tell an actual booking apart from
  someone merely opening the booking page. It is set `noindex, nofollow` so a
  search visitor cannot fire a false conversion. Point your Google Ads
  conversion at `makermindtutoring.com/booked.html`.
  To switch the redirect off and leave people on Calendly's own confirmation
  screen, set `scheduler.confirmPage` to `''` in `assets/config.js`.
  One trade-off: skipping Calendly's confirmation screen also skips its
  "add to calendar" buttons. The confirmation email still carries the
  calendar invite, and `booked.html` says so.

- **Page-visit conversions need Google's tag on the site.** Nothing here
  installs it, because it carries your account ID. Send me the tag snippet
  and I will add it to every page in one pass.

- **If a booking does not redirect to `booked.html`.** Open the booking page,
  press F12 (or Cmd+Option+J on a Mac) and look at the Console tab.
  - A line reading `[MakerMind] MakerMind scheduler v2` means the current
    `assets/scheduler.js` is deployed. **No such line means the host is still
    serving an older copy** — re-upload the `assets` folder and hard-refresh
    with Ctrl+Shift+R.
  - Lines reading `[MakerMind] Calendly event: ...` appear as you move
    through the calendar. If those never appear, Calendly is not reaching the
    page at all — check the event type's confirmation setting is
    **"Display confirmation page"**, not "Redirect to an external site".
    Calendly's own redirect shows the invitee a warning screen inside an
    inline embed, which is why the site handles it instead.
